...
import com.ashomapp.network.retrofit.getStockRetrofitService
...
object HomeRepository {
    ...
    private val stockService = getStockRetrofitService(HomeServices::class.java)
    ...
    suspend fun postSubscription(expire_data: String, subscription_type: String, amount: String) =
        safelyCallApi {
            services.postsubscription(SharedPrefrenceHelper.token.toString(),
                expire_data,
                subscription_type,
                amount)
        }
    ...
    suspend fun updateUserPro(
        ...
        profile_pic: Pair<ByteArray, String>? = null,
    ) = safelyCallApi {
        try {
            ...
            val token =
                MultipartBody.Part.createFormData("token", SharedPrefrenceHelper.token.toString())
    ...
    suspend fun changePassword(
        ...
        services.changepassword(SharedPrefrenceHelper.token.toString(),
            oldpass,
            newpass,
            confirmpass)
    }
    ...
    suspend fun getStockList() = safelyCallApi {
        stockService.getStockList()
    }
    suspend fun getStockCompanyNews(pageNo:Int,countryName: String,companyName: String) = safelyCallApi {
        services.getCompanyNews(pageNo.toString(),countryName,companyName)
    }

    suspend fun getStockCountryNews(pageNo:Int,countryName: String,search: String) = safelyCallApi {
        services.getNews(pageNo.toString(),countryName,search)
    }
    ...
    suspend fun getStockHistory(
        ...
        stockService.getStockHistory(days,
            weeks,
            months,
            year,
            spread,
